...
class Node
{
    ...
    Node * left,*right,*parent;//pointers to left and right children
...
class BST
{
    ...
    BST()
    {
        root=NULL; //root nullified for empty tree possibility
    }
    ...
    void insertHelper(Node * current, int value)//function to use recursion
    {
        ...
        else if(current->data < value)//insert in right side
        {
            if(current->right==NULL)//create a brand new node at the null position
            {
    ...
    {
        //check whether the value is equal to current node's data or if current is null
